...
void fake_data()
{
	...
	double true_v1 = 330.0;
	double true_v2 = 870.0;
	double true_k = 35.0;
	double true_sigma = 60.0;
	...
	for(int ii=0 ; ii<numdata ; ii++)
	{
		...
		/* calculate the mean y for this value of x */
		double mean;
		if(x<=true_k)
			mean = true_v1;
		else
	...
	// output fake data to file
	table_output("fakedata","./workspace/eg09_stepfunction_fakedata.txt");
...
void read_data()
{
	...
	table_read("mydata", "./workspace/eg09_stepfunction_fakedata.txt", 2);
...
void setup_parameters()
{
	...
	parameter_create("v1", -2000.0, 2000.0, 1000.0, 0, 0, 1);
	parameter_create("v2", -2000.0, 2000.0, 1000.0, 0, 0, 1);
	parameter_create("k", 0.0010, 100.0, 50.0, 0, 0, 1);
...
void likelihood()
{
	...
	for(int ii=0 ; ii<numdata ; ii++)
	{
		...
		table_writevalue("mydata","predy1",ii,predy);
		table_writevalue("mydata","predy2",ii,normal_draw(predy,cv("sigma")));
...
int main()
{
	...
	initialize_filzbach();
	name_analysis("eg09_stepfunction_00");
