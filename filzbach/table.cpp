...
#endif
		strcpy(mytable[tableindex].columnname[c], header);
	}

	// read values
	for (int r = 0; r < numrows; r++) // foreach row, skipping first row as already pointing to row 1. 
	{
		for(int c = 0; c < numcolumns; c++) // and each column
		{
			if (!feof(ifile)) // if there is still file left, read in tha values
			{
#ifdef WIN32
				fscanf_s(ifile, "%lf", &mytable[tableindex].data[c][r]);				
#else
				fscanf(ifile, "%lf", &mytable[tableindex].data[c][r]);				
#endif
			}
			else // fill the rest of the table  with missing values 
			{
				mytable[tableindex].data[c][r] = MISSING_VALUE;
			}
		}
	}		
...
{
//TODO!!	if (tid > -1 && table_get(tid) != NULL)
		return mytable[tid].numrows;	
